...
defaultConf		=	u'you-room@conference.server.tld'			# Стартовая конференция
...
base_host = '127.0.0.1' # хост базы для PostgreSQL
base_pass = '******'    # пароль базы для PostgreSQL

#-------------- Файлы, пути к файлам -----------#
slog_folder = 'log/%s'							# папка системных логов
set_folder 	= 'settings/%s'						# папка настроек
back_folder = 'backup/%s'						# папка хранения резервных копий
loc_folder 	= 'locales/%s'						# папка локализаций
log_folder 	= 'logs/%s'							# папка логов конференций
