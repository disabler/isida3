...
def gtalkers(type, jid, nick, text):
	if text:
		ttext = '%%%s%%' % text
		cur_execute('select * from talkers where (jid like %s or nick like %s or room like %s) order by -words',(ttext,ttext,ttext))
		tma = cur.fetchmany(10)
	else:
		cur_execute('select * from talkers order by -words')
		tma = cur.fetchmany(10)
	if tma:
...
def talkers(type, jid, nick, text):
	if text:
		ttext = '%%%s%%' % text
		cur_execute('select * from talkers where room=%s and (jid like %s or nick like %s) order by -words',(jid,ttext,ttext))
		tma = cur.fetchmany(10)
	else: 
		cur_execute('select * from talkers where room=%s order by -words',(jid,))
		tma = cur.fetchmany(10)
	if tma:
